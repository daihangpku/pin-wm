...
from diff_simulation.solver.util import get_sizes, bdiag
...
def forward(Q, p, G, h, A, b, F, Q_LU, S_LU, R, eps=1e-12, verbose=0, notImprovedLim=3,
    ...
    # else:
        # d = torch.ones(nBatch, nineq).type_as(Q)
        # factor_kkt(S_LU, R, d)
        # x = last_best_solutio["x"]
        # y = last_best_solutio["y"]

    # if last_best_solutio is not None :
    #     x = last_best_solutio["x"]
