...
def do_coacd(args, input_file):
    assert input_file.endswith(".obj") or input_file.endswith(".ply")
    if input_file.endswith(".obj"):
        output_file = input_file.replace(".obj", "_coll.obj")
    else:
        output_file = input_file.replace(".ply", "_coll.ply")
...
if __name__ == "__main__":
    ...
    if args.input.endswith(".urdf"):
        ...
        for urdf_link, transmat in fk.items():
            input_path = os.path.join(os.path.dirname(args.input), "parts", urdf_link.name + "_abs.obj")
            do_coacd(args, input_path)
    else:
        do_coacd(args, args.input)